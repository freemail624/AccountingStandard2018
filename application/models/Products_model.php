...
function get_product_history_with_child1($product_id,$depid=0,$as_of_date=null,$account,$is_parent=null,$ciaccount,$disaccount=null){
 ...
 function get_current_item_list($criteria=""){
            ...
            $sql="SELECT rc.*,p.*,u.unit_name,
                ...
                (
                    ...
                    -IFNULL(cinv.out_qty,0)
                    -IFNULL(iss.out_qty,0)
                    -IFNULL(aoQ.out_qty,0)

                ) as on_hand_per_batch
                    ...
                    CONCAT_WS('-',cii.batch_no,cii.product_id,cii.exp_date)as unq_id,
                    ...
                    WHERE 
                        ...
                        (p.product_desc LIKE '%".$criteria."%' OR p.product_code LIKE '%".$criteria."%' OR p.product_desc1 LIKE '%".$criteria."%' OR CAST(p.product_id AS CHAR) LIKE '%".$criteria."%') HAVING on_hand_per_batch>0";
...
*/
    ...
    function batch_inventory($product_id=0,$department_id=0,$count=null,$as_of_date){
        ...
        $sql="
            ...
            (
                ...
                SELECT 
                    ...
                    ON dii.dr_invoice_id=di.dr_invoice_id
                    WHERE di.is_active=TRUE AND di.is_deleted=FALSE
                    ".($product_id==0?"":" AND dii.product_id=".$product_id."")."
                ...
                UNION ALL
                    ...
                    INNER JOIN sales_invoice as si ON sii.sales_invoice_id=si.sales_invoice_id
                    WHERE si.is_active=TRUE AND si.is_deleted=FALSE
                    ".($product_id==0?"":" AND sii.product_id=".$product_id."")."
                ...
                UNION ALL
                    ...
                    SELECT 
                        ...
                        CONCAT_WS('-',cii.batch_no,cii.product_id,cii.exp_date)as unq_id,
                        0 as qty_in,
                        SUM(cii.inv_qty) as qty_out
                    FROM cash_invoice_items as cii
                    INNER JOIN cash_invoice as ci ON cii.cash_invoice_id=ci.cash_invoice_id
                    WHERE ci.is_active=TRUE AND ci.is_deleted=FALSE
                    ".($product_id==0?"":" AND cii.product_id=".$product_id."")."
                    ".($department_id==0?"":" AND ci.department_id=".$department_id."")."
                    ".($as_of_date==null?"":" AND ci.date_invoice <= '".$as_of_date."'")."                      
                    GROUP BY cii.product_id,cii.batch_no,cii.exp_date
                ...
                UNION ALL 
                    ...
                    FROM issuance_items as iss INNER JOIN issuance_info as iin ON iin.issuance_id=iss.issuance_id
                    WHERE iin.is_active=TRUE AND iin.is_deleted=FALSE
                    ".($product_id==0?"":" AND iss.product_id=".$product_id."")."
                ...
                UNION ALL
                    ...
                    ".($product_id==0?"":" AND idi.product_id=".$product_id."")."
         ...
         $sql="
            ...
            (
                ...
                UNION ALL
                    ...
                    WHERE ci.is_active=TRUE AND ci.is_deleted=FALSE
                    AND CONCAT_WS('-',cii.batch_no,cii.product_id,cii.exp_date) = '$unq_id'
                    ".($department_id==0?"":" AND ci.department_id=".$department_id."")."
                    ".($as_of_date==null?"":" AND ci.date_invoice <= '".$as_of_date."'")."                   
                    GROUP BY cii.product_id,cii.batch_no,cii.exp_date,ci.cash_invoice_id
