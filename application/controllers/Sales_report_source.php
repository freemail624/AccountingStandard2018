...
$excel->getActiveSheet()->setCellValue('A'.$i,'Invoice Source: '.$inv_src); $i++;$i++;
                ...
                foreach ($totals as $total) {
                    ...
                    $excel->getActiveSheet()->setCellValue('F'.$i,'Sub Total:');
                    $excel->getActiveSheet()->getStyle('F'.$i)->getFont()->setBold(True);
                    $excel->getActiveSheet()->getStyle('F'.$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
                    $excel->getActiveSheet()->setCellValue('G'.$i,number_format($total->sub_total,2));
                    $i++;
                    }
                }
                ...
                $i++;
                    ...
                    $excel->getActiveSheet()->setCellValue('F'.$i,'Grand Total:');
                    $excel->getActiveSheet()->getStyle('F'.$i)->getFont()->setBold(True);
                    $excel->getActiveSheet()->getStyle('F'.$i)->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
