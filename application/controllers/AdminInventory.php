...
class AdminInventory extends CORE_Controller
{
    ...
    public function index() {
        ...
        $data['title'] = 'Admin Inventory Report';

        $data['departments']=$this->Departments_model->get_list(array('is_deleted'=>FALSE,'is_active'=>TRUE));
        
        (in_array('15-9',$this->session->user_rights)? 
        $this->load->view('admin_inventory_report_view',$data)
    ...
    public function transaction($txn=null){
        switch($txn){
            ...
            case 'preview-inventory-with-total':
                ...
                $this->load->view('template/batch_inventory_report_with_cost',$data);
                break;
            ...
            case 'export-inventory-with-total':
                ...
                //name the worksheet
                $excel->getActiveSheet()->setTitle("Admin Inventory Report");
                $excel->getActiveSheet()->getStyle('A1')->getFont()->setBold(TRUE);
                $excel->getActiveSheet()->mergeCells('A1:B1');
                $excel->getActiveSheet()->mergeCells('A2:C2');
                $excel->getActiveSheet()->mergeCells('A3:B3');
                ...
                $excel->getActiveSheet()->setCellValue('A6','Admin Inventory Report - '.$department)
                                        ->getStyle('A6')->getFont()->setBold(TRUE);
                $excel->getActiveSheet()->setCellValue('A7','As of '.$date)
                                        ->getStyle('A7')->getFont()->setItalic(TRUE);
                $excel->getActiveSheet()->setCellValue('A8',$ccf_data)
                ...
                header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
                header('Content-Disposition: attachment;filename="Admin Inventory Report '.date('M-d-Y',NOW()).'.xlsx"');
                header('Cache-Control: max-age=0');
                // If you're serving to IE 9, then the following may be needed
                header('Cache-Control: max-age=1');
