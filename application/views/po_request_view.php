...
<!-- 
        ...
        <h2 class="h2-panel-heading">Purchase Requests<!-- <small> | <a href="assets/manual/purchasing/Purchase_Order.pdf" target="_blank" style="color:#999999;"><i class="fa fa-question-circle"></i></a></small> --></h2><hr>
            <table id="tbl_purchase_request" class="table table-striped" cellspacing="0" width="100%">
                <thead class="">
                <tr>
                    <th></th>
...
<div id="div_user_fields" style="display: none;">
    ...
    <div class="panel-body" >
        ...
        <form id="frm_purchase_requests" role="form" class="form-horizontal">
            <h2 class="h2-panel-heading">PR # : <span id="span_po_no">PR-XXXX</span></h2><hr>
            <div >
                <div class="row">
                    <div class="col-sm-5" >
                ...
                <table id="tbl_items" class="table table-striped" cellspacing="0" width="100%" style="font-font:tahoma;">
                    <thead>
                        <tr>
                            ...
                            <th class="hidden" width="1%">T.D</th>
                            <th class="hidden" width="1%">Tax %</th>
                            <th class="text-right" width="15%">Total</th>
                            <th class="hidden" width="1%">V.I</th>
                            <th class="hidden" width="1%">N.V</th>
                            <th class="hidden" width="1%">Item ID</th>
                            <th class="text-center" width="5%">Action</th>
                            <th class="hidden" width="1%">Bulk Price</th>
                            <th class="hidden" width="1%">Retail Price</th>
                        </tr>
                    </thead>
                    <tbody>
                    </tbody>
                    <tfoot>
                        ...
                        <tr>
                            ...
                            <td class="hidden">
                                <strong class="hidden"> <i class="glyph-icon icon-star"></i>Total After Discount :</strong>
                            </td>
                            <td id="td_total_after_discount" class="hidden">
                                <strong>0.00</strong></td><td style="border-right: 0px;">
                            </td>
                            <td colspan="2" align="right" style="border-left: 0px;">
                                <strong><i class="glyph-icon icon-star"></i> Total Before Tax :</strong>
                            </td>
                            <td align="right" colspan="2" id="td_before_tax">0.00</td>
                        </tr>
                        <tr>
                            <td colspan="2" align="right">
                            ...
                            <td align="right" colspan="2" id="td_after_tax">
                                0.00
                            </td>
                        </tr>
                    </tfoot>
...
<div id="modal_confirmation_close" class="modal fade" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">X</button>
                <h4 class="modal-title"><span id="modal_mode"> </span>Confirm closing of PR</h4>
...
$(document).ready(function(){
    ...
    var initializeControls=function(){
        ...
        dt=$('#tbl_purchase_request').DataTable({
            ...
            "columns": [
                ...
                { targets:[9],data: "purchase_request_id",visible:false }
            ]
        });

        var createToolBarButton=function(){
            var _btnNew='<button class="btn btn-primary"  id="btn_new" style="text-transform: capitalize;font-family: Tahoma, Georgia, Serif;" data-toggle="modal" data-target="" data-placement="left" title="New Purcahase Request" >'+
  ...
  /*          if(suggestion.is_bulk == 1){
    ...
    var bindEventHandlers=(function(){
        ...
        $('#tbl_purchase_request tbody').on( 'click', 'tr td.details-control', function () {
            ...
            if ( row.child.isShown() ) {
                ...
                $.ajax({
                    "dataType":"html",
                    "type":"POST",
                    "url":"Templates/layout/pr/"+ d.purchase_request_id,
        ...
        //         "dataType":"json",
        //         "type":"POST",
        //         "url":"Email/send/po/"+ d.purchase_request_id,
        ...
        $('#tbl_purchase_request tbody').on('click','#btn_email',function(){
            ...
            $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"Purchase_request/transaction/email/"+_selectedID,
        ...
        $('#tbl_purchase_request tbody').on('click','button[name="edit_info"]',function(){
            ...
            $.ajax({
                url : 'Purchase_request/transaction/items/'+data.purchase_request_id,
                type : "GET",
                cache : false,
                dataType : 'json',
                processData : false,
                contentType : false,
                beforeSend : function(){
                    $('#tbl_items > tbody').html('<tr><td align="center" colspan="15"><br /><img src="assets/img/loader/ajax-loader-sm.gif" /><br /><br /></td></tr>');
                },
        ...
        $('#tbl_purchase_request tbody').on('click','button[name="remove_info"]',function(){
            ...
            if(data.order_status_id != '1' ){
                showNotification({title:"Invalid",stat:"error",msg:"Only Open Purchase Request can be Deleted."});
            }else {
                _selectedID=data.purchase_request_id;
        ...
        $('#tbl_purchase_request tbody').on('click','button[name="mark_as_closed"]',function(){
            _selectRowObj=$(this).closest('tr');
            var data=dt.row(_selectRowObj).data();
            _selectedID=data.purchase_request_id;
        ...
        $('#btn_yes').click(function(){
            ...
            //}else{
            removePurchaseRequest().done(function(response){
                showNotification(response);
        ...
        $('#btn_save').click(function(){
            ...
            if(validateRequiredFields($('#frm_purchase_requests'))){
                if(_txnMode=="new"){
    ...
    var createPurchaseRequest=function(){
        var _data=$('#frm_purchase_requests,#frm_items').serializeArray();
    ...
    var updatePurchaseRequest=function(){
        ...
        _data.push({name : "summary_tax_amount", value : tbl_summary.find(oTableDetails.tax_amount).text()});
        _data.push({name : "summary_after_tax", value : tbl_summary.find(oTableDetails.after_tax).text()});
        _data.push({name : "purchase_request_id" ,value : _selectedID});
        _data.push({name : "remarks", value : $('textarea[name="remarks"]').val() });
    ...
    var MarkRecordAsClosed=function(){
        return $.ajax({
            "dataType":"json",
            "type":"POST",
            "url":"Purchase_request/transaction/close",
            "data":{purchase_request_id : _selectedID}
        });
    };    

    var removePurchaseRequest=function(){
        return $.ajax({
            "dataType":"json",
            "type":"POST",
    ...
    var newRowItem=function(d){
        ...
        return '<tr>'+
        '<td><input name="po_qty[]" type="text" class="numeric form-control number-keyup qty" value="'+ d.po_qty+'"></td>'+unit+
