...
<html lang="en">
                                ...
                                <div data-widget-group="group1">
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <div id="div_insurance_list">
                                                <div class="panel panel-default">
                                                    <div class="panel-body table-responsive">
                                                    <h2 class="h2-panel-heading">Insurance Management<!-- <small> | <a href="assets/manual/masterfiles/Salesperson_Management.pdf" target="_blank" style="color:#999999;"><i class="fa fa-question-circle"></i></a></small> --></h2><hr>
                                                        <table id="tbl_insurance" class="table table-striped" cellspacing="0" width="100%">
                                                            <thead class="">
                                                            <tr>
                                                                <th>Company</th>
                    ...
                    <div class="modal-dialog modal-md">
                        <div class="modal-content">
                            <div class="modal-header" style="background-color:#2ecc71;">
                                <button type="button" class="close"   data-dismiss="modal" aria-hidden="true">X</button>
                                <h4 id="insurance_title" class="modal-title" style="color: #ecf0f1;"><span id="modal_mode"></span></h4>
                            </div>
                            <div class="modal-body">
                                <div class="row">
    ...
    $(document).ready(function(){
        ...
        var initializeControls=function(){
            dt=$('#tbl_insurance').DataTable({
                "dom": '<"toolbar">frtip',
                "bLengthChange":false,
                "ajax" : "Insurance/transaction/list",
                "columns": [
                    { targets:[0],data: "insurer_company" },
            ...
            var createToolBarButton=function(){
                var _btnNew='<button class="btn btn-primary"  id="btn_new" style="text-transform: capitalize;font-family: Tahoma, Georgia, Serif;" data-toggle="modal" data-target="" data-placement="left" title="New Insurance" >'+
                    '<i class="fa fa-plus"></i> New Insurance</button>';
        ...
        var bindEventHandlers=(function(){
            ...
            $('#tbl_insurance tbody').on( 'click', 'tr td.details-control', function () {
            ...
            $('#btn_new').click(function(){
                ...
                $('#insurance_title').text('New Insurance');
                $('#modal_new_insurance').modal('show');
            });

            $('#tbl_insurance tbody').on('click','button[name="edit_info"]',function(){
                _txnMode="edit";
                _selectRowObj=$(this).closest('tr');
                var data=dt.row(_selectRowObj).data();
                _selectedID=data.insurance_id;
                ...
                $('#insurance_title').text('Edit Insurance');
                $('#modal_new_insurance').modal('show');
            });

            $('#tbl_insurance tbody').on('click','button[name="remove_info"]',function(){
                _selectRowObj=$(this).closest('tr');
                var data=dt.row(_selectRowObj).data();
                _selectedID=data.insurance_id;
            ...
            $('#btn_yes').click(function(){
                removeInsurance().done(function(response){
            ...
            $('#btn_save').click(function(){
                if(validateRequiredFields($('#frm_insurance'))){
                    if(_txnMode=="new"){
                        ...
                        updateInsurance().done(function(response){
                            showNotification(response);
        ...
        var updateInsurance=function(){
            var _data=$('#frm_insurance').serializeArray();
            _data.push({name : "insurance_id" ,value : _selectedID});

            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"Insurance/transaction/update",
                "data":_data,
                "beforeSend": showSpinningProgress($('#btn_save'))
            });
        };

        var removeInsurance=function(){
            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"Insurance/transaction/delete",
                "data":{insurance_id : _selectedID}
            });
        };

        var showList=function(b){
            if(b){
                $('#div_insurance_list').show();
                $('#div_insurance_fields').hide();
            }else{
                $('#div_insurance_list').hide();
                $('#div_insurance_fields').show();
            }
        };
