...
<html lang="en">
                                ...
                                <div data-widget-group="group1">
                                    <div class="row">
                                        <div class="col-md-12">
                                            ...
                                            <div id="div_advisor_list">
                                                <div class="panel panel-default">
                                                    <div class="panel-body table-responsive">
                                                    <h2 class="h2-panel-heading">Advisors<!-- <small> | <a href="assets/manual/masterfiles/Salesperson_Management.pdf" target="_blank" style="color:#999999;"><i class="fa fa-question-circle"></i></a></small> --></h2><hr>
                                                        <table id="tbl_advisor" class="table table-striped" cellspacing="0" width="100%">
                                                            <thead class="">
                                                            <tr>
                                                                <th>Code</th>
                                                                <th>Advisor</th>
                    ...
                    <div class="modal-dialog modal-md">
                        <div class="modal-content">
                            <div class="modal-header" style="background-color:#2ecc71;">
                                <button type="button" class="close"   data-dismiss="modal" aria-hidden="true">X</button>
                                <h4 id="advisor_title" class="modal-title" style="color: #ecf0f1;"><span id="modal_mode"></span></h4>
                            </div>
                            <div class="modal-body">
                                <div class="row">
    ...
    $(document).ready(function(){
        ...
        var initializeControls=function(){
            dt=$('#tbl_advisor').DataTable({
                "dom": '<"toolbar">frtip',
                "bLengthChange":false,
                "ajax" : "Advisors/transaction/list",
                "columns": [
                    { targets:[0],data: "advisor_code" },
            ...
            var createToolBarButton=function(){
                var _btnNew='<button class="btn btn-primary"  id="btn_new" style="text-transform: capitalize;font-family: Tahoma, Georgia, Serif;" data-toggle="modal" data-target="" data-placement="left" title="New Advisor" >'+
                    '<i class="fa fa-plus"></i> New Advisor</button>';
        ...
        var bindEventHandlers=(function(){
            ...
            $('#tbl_advisor tbody').on( 'click', 'tr td.details-control', function () {
            ...
            $('#btn_new').click(function(){
                ...
                $('#advisor_title').text('New Advisor');
                $('#modal_new_advisor').modal('show');
                _cboDepartment.select2('val',null);
            });

            $('#tbl_advisor tbody').on('click','button[name="edit_info"]',function(){
                _txnMode="edit";
                _selectRowObj=$(this).closest('tr');
                var data=dt.row(_selectRowObj).data();
                _selectedID=data.advisor_id;
                ...
                $('#advisor_title').text('Edit Advisor');
                $('#modal_new_advisor').modal('show');
            });

            $('#tbl_advisor tbody').on('click','button[name="remove_info"]',function(){
                _selectRowObj=$(this).closest('tr');
                var data=dt.row(_selectRowObj).data();
                _selectedID=data.advisor_id;
            ...
            $('#btn_yes').click(function(){
                removeAdvisor().done(function(response){
            ...
            $('#btn_save').click(function(){
                if(validateRequiredFields($('#frm_advisor'))){
                    if(_txnMode=="new"){
                        ...
                        updateAdvisor().done(function(response){
                            showNotification(response);
        ...
        var updateAdvisor=function(){
            var _data=$('#frm_advisor').serializeArray();
            _data.push({name : "advisor_id" ,value : _selectedID});

            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"Advisors/transaction/update",
                "data":_data,
                "beforeSend": showSpinningProgress($('#btn_save'))
            });
        };

        var removeAdvisor=function(){
            return $.ajax({
                "dataType":"json",
                "type":"POST",
                "url":"Advisors/transaction/delete",
                "data":{advisor_id : _selectedID}
            });
        };

        var showList=function(b){
            if(b){
                $('#div_advisor_list').show();
                $('#div_advisor_fields').hide();
            }else{
                $('#div_advisor_list').hide();
                $('#div_advisor_fields').show();
            }
        };
